...
async def query(cmd, params):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(p.rpc_url, data=command, headers={'content-type': "application/json", 'cache-control': "no-cache"}, auth=aiohttp.BasicAuth(p.rpc_usr, password=p.rpc_pass)) as resp:
                response = await resp.json()
    except Exception as E:
        logging.warning('Exception triggered in communication with PINK client: %s', E)
        logging.warning('CMD: %s ARGS: %s', cmd, params)
        return 3
    if response['error'] != None:
        if response['error']['code'] == -17:
            return None
        logging.warning('Error response sent by PINK client: %s', response['error'])
        logging.warning('CMD: %s ARGS: %s', cmd, params)
        return 1
    else:
...
async def unlock():
    return await query('walletpassphrase', [p.pink_pass, 999999999, False])
