...
def amt_filter(inp):
    try:
        inp = float(inp)
        if (inp < 0) or (inp <= p.MIN_TX) or not isfinite(inp):
            return None
        else:
            return round(inp, 8)
    except:
...
Faucet funds: {} PINK ({})
Rain Funds: {} PINK ({})```'''.format(e.ONLINE, p.tx_fee, p.MIN_TX, p.tx_timeout,
p.FCT_REQ_LIM, len(await q.get_addr_uid_dict()), block_height, block_hash,
...
async def donate(user_obj, selection, amount):
    selection -= 1
    if 0 <= selection < len(p.donation_accts):
        acct_dict = p.donation_accts[selection]
        selection_name = list(acct_dict.keys())[0]
        addr = acct_dict[selection_name]
        result = await user_obj.withdraw(amount, addr, p.net_fee, True)
...
async def rdonate(user_obj, amount):
    return await donate(user_obj, r.randint(1, len(p.donation_accts)), amount)
...
async def faucet(uid):
    ...
    amount = round(r.uniform(p.FCT_MIN, p.FCT_MAX), 8)
    user_obj, exit_code = await q.faucet_operations(uid, amount, ctime)
    if exit_code == 1:
        return '{}Request too recent. Faucet timeout is {} hours. Try again in: {}'.format(e.CANNOT, p.FCT_REQ_LIM,
    ...
    return docs.claim.format(e.GOOD, amount) + ' Your new balance is `{} PINK`'.format(round(user_obj.balance, 8))
...
async def burn_coins(args):
    ...
    if args[0] != p.owner_id and (not from_user is None):
        to_user = await q.get_user(p.owner_id)
        await from_user.send_internal_tx(to_user, amt)
    else:
        from_user.balance -= amt
        await q.save_user(from_user)
    return 'Burned `{} PINK` from `{}`'.format(amt, args[0])
