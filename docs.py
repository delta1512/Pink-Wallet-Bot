...
### GENERAL MESSAGES
welcome = '{}Welcome to the PINK Wallet Bot! Type `!help` for more commands and be sure to read the `!terms` and `!rules`'.format(e.CELEBRATE)

faucet_msg = '''The faucet currently contains `{} PINK`.

Donate PINK to this address: `{}` or use `!fgive` to refill the faucet.'''

rain_msg = '''The rainbot currently contains `{} PINK` and will rain at `{} PINK`.

Donate PINK to this address `{}`
or type `!rain [amount-PINK]` to build up rain.'''
...
rain_thankyou = '\n\nThank you for raining on the users! The rain pot is now at `{} PINK`'

donation_recipient = '\nYou donated to {}!'

info = discord.Embed(title='This bot is the original work of Delta and various contributors.', colour=discord.Colour.orange(),
description='''
The source code for the bot can be found [here](https://github.com/delta1512/PINK-Wallet-Bot).
...
claim = '{}You claimed `{} PINK` from the faucet!'


### SUCCESS MESSAGES
new_user_success = '{}User account created successfully. Your address is `{}`'

net_tx_success = '{}Transaction of `{} PINK` (inc. {} PINK fee) was successful, ID: `{}`{}'

internal_tx_success = '{}In-server transaction of `{} PINK` was successful.'
...
more_than_fee_and_min = '{}You must provide an amount that is greater than the fee and minimum (`{} PINK`).'.format(e.ERROR, p.tx_fee + p.MIN_TX)
...
too_new_msg = '{}Your account is too new to be using the bot, please ensure your account is at least {} days old.'.format(e.CANNOT, p.NEW_USR_TIME)


### RULES AND TERMS
rules = discord.Embed(title='PINK Wallet Bot Rules', colour=discord.Colour.purple(),
description='''
...
terms = discord.Embed(title='PINK Wallet Bot Terms and Conditions', colour=discord.Colour.purple(),
description='''
