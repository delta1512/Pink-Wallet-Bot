...
handler = [RotatingFileHandler(p.log_dir+'walletbot.log', maxBytes=10**7, backupCount=3)]
logging.basicConfig(format='[%(asctime)s] %(levelname)s: %(message)s',
                    datefmt='%d/%m %T',
                    level=logging.INFO,
                    handlers=handler)

client = commands.Bot(command_prefix=p.pre)
...
def limit_to_main_channel():
    def predicate(ctx):
        return str(ctx.channel.id) in p.main_channels or isinstance(ctx.channel, discord.DMChannel)
    return commands.check(predicate)


def is_owner():
    def predicate(ctx):
        return str(ctx.author.id) == p.owner_id
    return commands.check(predicate)
###
...
async def on_ready():
    ...
    if await w.query('getblockcount', []) > 5:  # 5 is largest error return value
        logging.info('Pinkcoin client is online')
    else:
        logging.error('Pinkcoin client is not online')
        await client.logout()
        return

    if await q.uid_exists(FCT):
    ...
    if await w.unlock() is None:
        ...
        logging.error('There was a problem trying to unlock the Pinkcoin wallet')
        await client.logout()
        return
...
async def withdraw(ctx, address: str, amount: float):
    ...
    await ctx.send(await user_obj.withdraw(extras.amt_filter(amount), address, p.tx_fee))
...
async def on_message(msg):
    ...
    uname = a.name
    user = str(a.id)
    iscommand = cmd.startswith(p.pre)
    ...
    # Check for new user
    if iscommand and time() < user_time(a.created_at)+24*60*60*p.NEW_USR_TIME:
